...
namespace SQLite.Helper
{
    public class DBHelper : SQLiteOpenHelper
    {
        ...
        private static string DB_NAME = "Spelers.db";
        ...
        private SQLiteDatabase CreateSQLiteDB()
        {
            ...
            try
            {
                ...
                {
                    streamSQLite = context.Resources.OpenRawResource(Resource.Raw.Spelers);
        ...
        private bool CopySQLiteDB(Stream streamSQLite, FileStream streamWriter)
        {
            ...
            try
            {
                int byteRead = streamSQLite.Read(buffer, 0, length);
                while(byteRead > 0)
                {
                    streamWriter.Write(buffer, 0, byteRead);
                    byteRead = streamSQLite.Read(buffer, 0, length);
